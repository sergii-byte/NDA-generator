...
// ═══════════════════════════════════════════════════════════
// Estonia — ariregxmlv6.rik.ee SOAP API (FREE with contract)
// Docs: https://avaandmed.ariregister.rik.ee/en/open-data-api/enterprise-simple-data-request-status-query
// Query: lihtandmed_v2 — search by name (evnimi), returns up to 100 results
// Env vars: ARIREGISTER_USERNAME, ARIREGISTER_PASSWORD
// Fallback: autocomplete endpoint (may be blocked by Cloudflare)
// ═══════════════════════════════════════════════════════════
async function searchEstoniaAriregister(query) {
  const username = process.env.ARIREGISTER_USERNAME;
  const password = process.env.ARIREGISTER_PASSWORD;

  // Primary: SOAP API (no Cloudflare, reliable)
  if (username && password) {
    try {
      return await searchEstoniaSOAP(query, username, password);
    } catch (e) {
      // Fall through to autocomplete
    }
  }

  // Fallback: autocomplete (may be blocked by Cloudflare on some hosts)
  return await searchEstoniaAutocomplete(query);
}

// ═══════ Estonia SOAP API — ariregxmlv6.rik.ee ═══════
async function searchEstoniaSOAP(query, username, password) {
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:prod="http://arireg.x-road.eu/producer/">
  <soapenv:Body>
    <prod:lihtandmed_v2>
      <prod:keha>
        ...
        <prod:evnimi>${escapeXml(query)}</prod:evnimi>
        <prod:evarv>15</prod:evarv>
  ...
  try {
    const res = await fetch('https://ariregxmlv6.rik.ee/', {
      method: 'POST',
      headers: {
    ...
    // Parse XML response — extract <ns1:item> blocks
    const items = [];
    ...
    while ((match = itemRegex.exec(xml)) !== null) {
      const block = match[1];
      const get = (tag) => {
        const m = block.match(new RegExp(`<ns1:${tag}>([\\s\\S]*?)<\\/ns1:${tag}>`));
        return m ? m[1].trim() : null;
      };

      const name = get('evnimi');
      const regCode = get('ariregistri_kood');
      if (!name || !regCode) continue;

      // Build address from normalized field or parts
      let address = get('aadress_ads__ads_normaliseeritud_taisaadress');
      if (!address) {
        const street = get('asukoht_ettevotja_aadressis');
        const city = get('asukoha_ehak_tekstina');
        const zip = get('indeks_ettevotja_aadressis');
      ...
      const statusCode = get('staatus');
      const statusText = get('staatus_tekstina');
      const statusMap = { 'R': 'Registered', 'K': 'Deleted', 'L': 'In liquidation', 'N': 'Being founded' };
      const status = statusMap[statusCode] || statusText || 'Active';

      const legalForm = get('oiguslik_vorm_tekstina') || get('oiguslik_vorm');
      const legalFormMap = {
        'OÜ': 'OÜ', 'Osaühing': 'OÜ',
        'AS': 'AS', 'Aktsiaselts': 'AS',
        'FIE': 'FIE', 'Füüsilisest isikust ettevõtja': 'FIE',
        'MTÜ': 'MTÜ', 'Mittetulundusühing': 'MTÜ',
        'SA': 'SA', 'Sihtasutus': 'SA',
        'TÜ': 'TÜ', 'Täisühing': 'TÜ',
        'UÜ': 'UÜ', 'Usaldusühing': 'UÜ',
      };
      const companyType = legalFormMap[legalForm] || legalForm || null;
      ...
      items.push({
        ...
        companyType,
        incorporationDate: get('esmakande_aeg') || null,
...
// ═══════ Estonia Autocomplete fallback ═══════
async function searchEstoniaAutocomplete(query) {
  const endpoints = [
