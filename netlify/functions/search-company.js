exports.handler = async (event, context) => {
  ...
  try {
    ...
    // ═══════ PARALLEL SEARCH — 3 calls ═══════
    // 1. UK Companies House   (direct API, API key required)
    // 2. Czech ARES            (free REST API, no auth)
    // 3. OpenCorporates        (single call, per_page=15, covers 140+ countries incl. Estonia)
    // Note: Estonia ariregister blocked by Cloudflare from server — users search manually

    const [ukResults, czResults, ocResults] = await Promise.all([
    ...
    let results = [...ukResults, ...czResults, ...ocResults];
